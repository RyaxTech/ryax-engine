...
COLUMNS="$COLUMNS,GPUALLOC:.status.allocatable.nvidia\\.com/gpu"
...
while true; do
  ...
  echo "$nodes" | while read -r name taints ready mig migstate gpualloc; do
    [ -n "${name:-}" ] || continue
    ...
    # And the device plugin has to be advertising the devices. It registers
    # once when it starts, so on a MIG node it first publishes the whole cards,
    # then drops to zero while MIG Manager reconfigures, and only advertises the
    # slices after the operator restarts it -- which lands *after*
    # mig.config.state turns success. Releasing on the label alone hands back a
    # node that cannot yet run anything.
    case "$gpualloc" in
      ""|"<none>"|0)
        echo "$name: MIG configured, no allocatable GPU yet, waiting"
        continue ;;
    esac
