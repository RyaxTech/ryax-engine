...
| promtail | object | `{"config":{"clients":[{"url":"{{ printf \"http://%s-loki:3100/loki/api/v1/push\" .Release.Name }}"}],"snippets":{"extraRelabelConfigs":[{"action":"labelmap","regex":"__meta_kubernetes_pod_label_(.+)"},{"action":"labeldrop","regex":"app_kubernetes_io_(.+)"}]}},"serviceMonitor":{"enabled":true},"tolerations":[{"effect":"NoSchedule","operator":"Exists"}]}` | Promtail is an external dependency that provides log collection on Kubernetes. |
| promtail.config.clients[0] | object | `{"url":"{{ printf \"http://%s-loki:3100/loki/api/v1/push\" .Release.Name }}"}` | Required to match the release name |
| promtail.config.snippets.extraRelabelConfigs | list | `[{"action":"labelmap","regex":"__meta_kubernetes_pod_label_(.+)"},{"action":"labeldrop","regex":"app_kubernetes_io_(.+)"}]` | Drop labels to avoid the "too many labels" error |
| promtail.tolerations | list | `[{"effect":"NoSchedule","operator":"Exists"}]` | Be sure that Promtail runs on every node |
