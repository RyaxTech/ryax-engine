...
This script creates the image archive and the Helm packages required for the installation. Additionally, you need to copy the two following files for your airgap installation:
...
- [minimal.yaml](https://raw.githubusercontent.com/RyaxTech/ryax-engine/refs/heads/master/charts/ryax/env/minimal.yaml)

!!! note
    The image list is generated by rendering the charts, so a feature that is
    off by default contributes no images. If you intend to use the
    [GPU readiness gate](./gpu_node_pools.md) on this cluster, mirror
    `registry.k8s.io/node-problem-detector/node-problem-detector` as well and
    point `gpuReadiness.nodeProblemDetector.image` at your copy —
    `global.imageRegistry` does not rewrite that key.
